-- Cleanup script for Supabase - FIXED FK ORDER (cost_events before heartbeat_runs)
BEGIN;
...
-- Step 2: Delete cost_events BEFORE heartbeat_runs (FK constraint!)
DELETE FROM cost_events WHERE company_id IN (SELECT id FROM companies WHERE status = 'archived');

-- Step 3: Delete heartbeat_runs for archived companies/agents
...
-- Step 4: Delete agent-related records
...
-- Step 5: Delete issue-related records
...
-- Step 8: Delete remaining company-related records
