/**
 * Migration v4: Skip QualityForge (already correct), scope PromptForge to its company only
 */
...
const AGENT_FILES = [
  ...
  "agents-loopy.json",
  // REMOVED: "agents-qualityforge.json" — already correct in DB, don't overwrite
];
...
// PromptForge agents - ALL report to Sage (CEO)
...
function generateMigrationSql(
  ...
  lines.push("-- Org hierarchy migration v4");
  lines.push("-- Covers: DomainForge, DeepForge, DevForge, InfraForge, Loopy, PromptForge");
  lines.push("-- SKIPS QualityForge (already correct in DB)");
  ...
  // Pass 3: PromptForge - adapterConfig (scoped to company_id)
  lines.push("-- 3. PromptForge adapterConfig (company_id = '${PROMPTFORGE_COMPANY_ID}')");
  for (const agent of promptforgeAgents) {
    ...
    lines.push(`  ELSE jsonb_set(jsonb_set(adapter_config::jsonb, '{authority_level}', '"${authorityLevel}"'::jsonb), '{division}', '"${division}"'::jsonb)`);
    lines.push(`END`);
    lines.push(`WHERE company_id = '${PROMPTFORGE_COMPANY_ID}' AND name ILIKE '${escapeRegex(agent.name)}';`);
    lines.push("");
  }

  // Pass 4: PromptForge - reports_to (scoped to company_id)
  lines.push("-- 4. PromptForge reports_to (company_id = '${PROMPTFORGE_COMPANY_ID}')");
  for (const agent of promptforgeAgents) {
    if (agent.reportsTo) {
...
function main() {
  ...
  console.log(`Agents from JSON configs (NO QualityForge): ${agents.length}`);
  ...
  const outputPath = path.resolve(__dirname, "../tmp/org-hierarchy-migration-v4.sql");
  if (!fs.existsSync(path.dirname(outputPath))) fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, sql);
  console.log(`\nSQL written to: ${outputPath}`);
  console.log("Apply via Supabase SQL Editor or psql.");
  console.log("\nNOTE: QualityForge is SKIPPED — it was already correct in the database.");
}
